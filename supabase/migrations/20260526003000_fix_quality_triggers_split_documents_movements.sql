-- Corregge il trigger qualità dati separando documenti e movimenti.
-- Motivo: documents usa data_documento, accounting_movements usa data.
-- La funzione unica precedente poteva bloccare update sui documenti.
...
drop function if exists public.normalize_private_area_quality_fields();

create or replace function public.normalize_document_quality_fields()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  new.lavorazione := nullif(coalesce(new.lavorazione, original_category, new.sheet_tab), '');
  ...
  if original_category = 'Alloggi' or new.sheet_tab = 'Alloggi' then
    new.natura_movimento := 'Alloggio';
  elsif original_category = 'Vitto' or new.sheet_tab = 'Vitto' then
    new.natura_movimento := 'Vitto';
  elsif original_category = 'FIR / Rifiuti' or new.sheet_tab ilike '%rifiuti%' then
  ...
  elsif coalesce(new.tipo_documento,'') ilike '%fattura%' then
    new.natura_movimento := 'Fattura';
  elsif coalesce(new.tipo_documento,'') ilike '%ricevuta%' then
    new.natura_movimento := 'Ricevuta';
  else
    new.natura_movimento := coalesce(new.natura_movimento, 'Documento operativo');
  end if;
  ...
  new.data_quality_note := nullif(concat_ws(' | ',
    ...
    case when new.lavorazione is not null then 'Lavorazione: ' || new.lavorazione end,
    case when new.data_documento > current_date then 'Data futura/sospetta' end,
  ...
  if new.data_documento > current_date
...
create or replace function public.normalize_movement_quality_fields()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  new.lavorazione := nullif(coalesce(new.lavorazione, original_category, tab_name), '');
  delta := round((coalesce(new.imponibile,0) + coalesce(new.iva,0) - coalesce(new.totale,0))::numeric, 2);
  ...
  new.data_quality_note := nullif(concat_ws(' | ',
    case when original_category is not null and original_category <> new.categoria then 'Categoria normalizzata da: ' || original_category end,
    case when new.lavorazione is not null then 'Lavorazione: ' || new.lavorazione end,
    case when new.data > current_date then 'Data futura/sospetta' end,
    case when coalesce(new.descrizione,'') ilike '%fattura da collegare%' or coalesce(new.note,'') ilike '%fattura da collegare%' then 'Pagamento da collegare a fattura' end,
    case when abs(delta) > 0.02 then 'Delta matematico: ' || delta::text end
  ), '');
...
for each row execute function public.normalize_document_quality_fields();

create trigger normalize_accounting_movements_quality_fields
before insert or update on public.accounting_movements
for each row execute function public.normalize_movement_quality_fields();
