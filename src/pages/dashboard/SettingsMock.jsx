...
export function SettingsMock({ session, store }) {
  ...
  async function createInvite(event) {
    ...
    try {
      ...
      const invite = {
        ...
        token,
        inviteUrl: normalizeActionLink(result?.actionLink),
  ...
  async function getRealInviteUrl(invite) {
    if (isRealSupabaseInviteUrl(invite.inviteUrl)) return normalizeActionLink(invite.inviteUrl)
...
function loadInvites() {
  try {
    ...
    return parsed.map((invite) => ({
      ...
      inviteUrl: normalizeActionLink(invite.inviteUrl),
...
function normalizeActionLink(url) {
  if (!url || typeof url !== 'string') return url
  try {
    const parsed = new URL(url)
    const redirectTo = parsed.searchParams.get('redirect_to')
    if (redirectTo && (redirectTo.includes('localhost') || redirectTo.includes('127.0.0.1'))) {
      parsed.searchParams.set('redirect_to', 'https://xvxexe.github.io/hub/#/dashboard/login')
      return parsed.toString()
    }
    return url
  } catch {
    return url.replace('http://localhost:3000/#', 'https://xvxexe.github.io/hub/#')
  }
}
